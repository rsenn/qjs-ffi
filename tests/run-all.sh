#!/bin/sh
# Runs every tests/test-*.js suite with qjsm and reports overall pass/fail.
# tinytest.js doesn't set a process exit code on failure, so this greps its
# own summary line instead.
#
# Usage: tests/run-all.sh [qjsm-binary]
